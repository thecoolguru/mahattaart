...
function Multiple(rowId1,rowId2,rowId3){
    ...
    function DynamicContent(divId,QueryPage){
     ...
     function calculator_offline()
                    {

                        
                        var packaging_charge_off = document.getElementsByName('packaging_charge_off[]');
                         var courier_charge_off = document.getElementsByName('courier_charge_off[]');
                        var license_cost1 = document.getElementsByName('license_cost_off[]');
                   var category = document.getElementsByName('category_off[]');
         var border_off1 = document.getElementsByName('border_off[]');
                        ...
                        for(var i=0;i<=print_height1.length;i++)
                            {
                              ...
                              var val= category[i].value;
                               var packaging_charge= packaging_charge_off[i].value;
                              var courier_charge= courier_charge_off[i].value;
				 ...
				 var org_license_cost=border_cost*license_cost_set/100;
					
                                    $('#license_cost_off'+[i]).html(org_license_cost);
                                     if(!isNaN(border_cost) && border_cost!=""){
                                    $('#print_cost_off'+[i]).val(border_cost);
                                     }
                               ...
                               $('#mount_cost_off'+[i]).val(OrgMountCost);
                                ...
                                var OrgFrametRuningcost = ((parseFloat(OrgMountHeight)+parseFloat(frame_width_values)*2)*2) +((parseFloat(OrgMountWidth)+ parseFloat(frame_width_values*2))*2);
                                var OrgFramCost=parseInt((OrgFrametRuningcost)/(12)*65);//rate 65
                               $('#frame_cost_off'+[i]).val(OrgFramCost);
                               //var mountarea=parseFloat(print_height_values)* parseFloat(print_width_values)*2;
                               var glass_cost=OrgMountArea*.38;// rate 2
             ...
             if(frame_width_values!='')
                                ...
                                var totalCost;
                               if(val=='1'){
                                 totalCost=parseInt(border_cost);
                               }
                                   if(val=='2'){
                                totalCost=parseInt(OrgMountCost)+parseInt(border_cost);
                           }
                                   if(val=='3'){
                                totalCost=parseInt(OrgFramCost)+parseInt(border_cost);
                           }
                                   if(val=='4'){
                                totalCost=parseInt(OrgMountCost+OrgFramCost+glass_cost)+parseInt(border_cost);
                           } if(val=='5'){
                                totalCost=parseInt(OrgMountCost+OrgFramCost)+parseInt(border_cost);
                           } if(val=='6'){
                                totalCost=parseInt(OrgFramCost+canvas_cost+parseInt(border_cost)
                                        );
                           }
                                  ...
                                  if (!isNaN(totalCost)){
                                    document.getElementById("total_off"+[i]).value = totalCost+parseInt(packaging_charge)+parseInt(courier_charge);
                                    }
                          ...
                          // alert(final_values);
                           var _tax=final_values*5/100;
                               ...
                               $('#finalTotal_txt_off_cp').val(after_tax_val);
                                }
