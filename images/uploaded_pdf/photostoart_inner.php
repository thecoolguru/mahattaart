	 ...
	 if( size == 'Customize Size'){
	   ...
	   /* function addToCart(){
		...
		if($('#click').html() == 'canvas_click'){
			...
			framed = framed.split('"');
			frameheight = framed[1].split('X');
			framed_art = ''+framed[0]+'X'+frameheight[1]; 
		}else{
			var framed = $('#finished_size').html();
			framed = framed.split('"');
			frameheight = framed[1].split('X');
			...
			var img_name = $('#get_img').val();
			img_name = img_name.split('/');
			var image_name = img_name[7];
			var print_price = $('#print_price').html();
			var total_price = $('.actual_price').html();
			$.ajax({
				 type: "POST",
				 url: "<?=base_url()?>index.php/frontend/photostoart_addtocart",
				 data: {print_type:paper_surface, img_id:image_id, frame_size:framed_art, img_name:image_name, date_time:date, print:print_price , total:total_price},
				 success:function(data)  
				 {    
				 	/* $('.frame-step-header-container').show();
					feedback_of_addtocart(data);
