...
//print_r($invoice_id);die;
	...
	public function vendor_location() 
	{
		...
		//echo "in controller".$vendor_type; die();
        $result2=$this->frontend_model->get_kiosk_location($vendor_type);
		//print_r($result2);
		echo '<option>--Select--</option>';
	...
	public function vendor_location_id()
	 {
		...
		//echo "in controller=".$vendor_location; die();
		$result3=$this->frontend_model->get_kiosk_locationid($vendor_location);
    	foreach ($result3 as $data3) 
		{
			?>
            <option value="<?php echo  $data3->location_id?>"><?php   echo $data3->location_id;?></option>
            <?php
    	}
	}
...
public function themes_lightbox($lightbox_id,$page_no,$category="none",$shape="none",$offset=0)	{
	...
	public function register()	
	{
		...
		//GET VENDOR TYPES,LOCATION,LOCATION ID
				  /****Create seqence id**/   
		...
		if($maill==1)	
		{
			...
			$quote="'";
		 $result=$this->frontend_model->insert_registeration($vendor_type,$vendor_location,$vendor_location_id,$first_name,$last_name,$email,$password,$company_name,$ima,$job_dec_details,$job_dec,$final_id);
	...
	public function frameit_addtocart()	{
		...
		$print_size=$this->input->post('images_size');
		$final_sizes=$this->input->post('final_sizes');
		//echo $final_sizes; die();
		$print_size=$this->input->post('images_size');
		$images_price=$this->input->post('images_price');
		$img_id=$this->input->post('img_id');
		$user_id=$this->input->post('user_id');
		...
		if($img_id!=''){
			if($print_value!='')	{
				...
				$res=array('cart_id'=>'', 'image_print_type'=>$paper_surface, 'image_id'=>$img_id, 'qty'=>1, 'user_id'=>$user_id, 'frame_size'=>$frameSize, 'frame_color'=>$frame_color, 'frame_cost'=>$FrameCost, 'mount_size'=>$mat_size,'mount_code'=>$mount_color, 'mount_color'=>$mat_color, 'mount_cost'=>$MountCost, 'glass_type'=>$glasses, 'glass_cost'=>$glasses_coste, 'price'=>$total_price,'total_price'=>$total_price, 'updated_price'=>'', 'image_size'=>$print_size,'framed_image_size'=>$final_frame_size,'images_price'=>$images_price, 'image_name'=>$images_filename, 'create_date'=>$date, 'path'=>$path,'size'=>$product_size,'promo_code'=>$promo_code,'promo_discount'=>$promo_discount,'promo_price'=>$promo_price,'old_price'=>$old_price,'image_final_size'=>$final_sizes);
				$data=array_map('trim',$res);
				$check1= $this->frontend_model->check_cart_details($user_id,trim($img_id),trim($paper_surface),$print_size,$frame_color,trim($mat_color),trim($glasses));
			}
			//print_r($data);
