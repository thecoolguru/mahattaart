...
public function get_location_id_for_promo_code()
{
	...
	$prom_code_status = $this->input->post('prom_code_status');
	...
	$valid_days      = $this->input->post('valid_days');
	$promo_condition = trim($this->input->post('promo_condition'));
	$status=0;
	$data=array(
	            'prom_for'=>$prom_for,
				'location'=>$location,
	...
	if($this->form_validation->run()==false)
	{
		...
		/*Update Expired Prormo Code*/ 
		if( !empty($promo_code_id) && $status_id==2)
		{		
		  
		 
		  $query=$this->backend_model->get_all_promo_code_details_model($promo_code_id,$status_id);
		  //echo $query[0]->valid_from_date; die();
	
		  if(isset($date_from_text)){$new_updated_from_date=$date_from_text;}else{$new_updated_from_date=$query2[0]->valid_from_date;}
		  if(isset($date_to_text)){$new_updated_end_date=$date_to_text;}else{$new_updated_end_date=$query2[0]->valid_end_date;}
		  ...
		  $expired=array(
				...
				'promo_name_code'=>$promo_code,
				'offer_precentage'=>$offer_precentage,
				'valid_from_date'=>$new_updated_from_date, 
				'valid_end_date'=>$new_updated_end_date,
				'promo_condition'=>$promo_condition,
				'valid_days'=>$valid_days,
				'active'=>$prom_code_status
	           );
		    $query['record_added']=$this->backend_model->update_expired_prormo_code_model($promo_code_id,$status_id,$expired);
		    $query['query_message']="Expired Promocde Successfully Updated.";
			
		}
	/*Update Deactive Prormo Code*/ 
	
	if( !empty($promo_code_id) && $status_id==0)
	{
	   $query=$this->backend_model->get_all_promo_code_details_model($promo_code_id,$status_id);
	   if(isset($date_from_text)){$new_updated_from_date=$date_from_text;}else{$new_updated_from_date=$query[0]->valid_from_date;}
		  if(isset($date_to_text)){$new_updated_end_date=$date_to_text;}else{$new_updated_end_date=$query[0]->valid_end_date;}
	  ...
	  $deactive=array(
		    ...
		    $query['record_added']=$this->backend_model->update_deactive_prormo_code_model($promo_code_id,$status_id,$deactive);
		    $query['query_message']="Deactive Promocde Successfully Updated.";
	}
	
	
	/*Update Active Promo Code*/
	
	if( !empty($promo_code_id) && $status_id==1)
	{
	   $query=$this->backend_model->get_all_promo_code_details_model($promo_code_id,$status_id);
	      if(isset($date_from_text)){$new_updated_from_date=$date_from_text;}else{$new_updated_from_date=$query[0]->valid_from_date;}
		  if(isset($date_to_text)){$new_updated_end_date=$date_to_text;}else{$new_updated_end_date=$query[0]->valid_end_date;}
	   $active=array(
		    ...
		    $query['record_added']=$this->backend_model->update_active_prormo_code_model($promo_code_id,$status_id,$active);
		    $query['query_message']="Active Promocde Successfully Updated.";

	}

	/*Add a New Prormocode */	 
  if(empty($promo_code_id) && empty($status_id))
		 {
...
public function promo_code_deletions()
{
	$promo_code_id=$this->uri->segment(3);
	$this->backend_model->promo_code_deletions_model($promo_code_id);
   redirect('backend/manage_promo_offer/'.$promo_code_id);
}
