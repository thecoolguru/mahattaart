...
function createTree($array, $currentParent, $currLevel = 0, $prevLevel = -1) {
    foreach ($array as  $categoryId => $category) {
     ...
     // echo $category['id'];
         ...
         //if($_REQUEST['id']==$category['id']){ echo 'asdsadas';}
        if ($currentParent == $category['p_id']) {
            ...
            {?>
<li>  <label  style="color:red;" for ="subfolder2" ><?=$category['name'];?> &nbsp &nbsp <a href="view_editcategory/<?=$category['id'];?>">Edit </a>  &nbsp  <a href="add_category/<?php echo $category['id'].'/'.$category['name'];?>"> Add </a>&nbsp <a href="view_images/<?=$category['id'];?>">show_images </a>  </label> <input type="checkbox"  id="subfolder2" />
            <?php }
            if($category['status']==1)
            {?>
               <li>  <label   for ="subfolder2" ><?=$category['name'];?> &nbsp &nbsp <a href="view_editcategory/<?=$category['id'];?>">Edit </a>  &nbsp  <a href="add_category/<?php echo $category['id'].'/'.$category['name'];?>"> Add </a>&nbsp <a href="view_images/<?=$category['id'];?>">show_images </a>  </label> <input type="checkbox"   <?php if($_REQUEST['id']==$category['id']){?> checked <?php }?>  id="subfolder2" value="<?=$category['id']?>"/>
            <?php }           
