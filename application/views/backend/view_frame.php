...
<div class="main-hdr"> View Framer Details</div>
...
<div class="customer-list">
    ...
    Total Framer:<span class="count">&nbsp;<?=  mysql_num_rows($rows);?></span>  
<a href="<?=base_url()?>index.php/backend/add_framer" class="addnewcp">Add New Framer</a></div>
<div style="height:25px;"> </div>
<div class="viewcplist-inner">
    <table width="100%"  id="printer_details" class="display" >
        <thead>
        <tr class="hdrs">
          <td width="150">Framer code  </td>
          <td width="150">Contact Person  </td>
          <td width="60">Display Name  </td>
          <td width="150">Area </td>
          <td width="120">Cost</td>
          <td width="120">Rate</td>
          <td width="120">Create Date</td>
          <td>Action</td>
          </tr>
        </thead>
          ...
          while($data = mysql_fetch_object($framer_code)){
              //$Result=$this->backend_model->Get_all_frames_data($data->framer_code);
              $sql="select * from tbl_framer where contact_person_name='".$data->contact_person_name."' order by framer_id desc ";
             $rows=  mysql_query($sql);
           $Result=   mysql_fetch_object($rows);
              ?>
         <tbody>
          <tr>
          <td><?=$Result->framer_code;?></td>
          <td><?=$Result->contact_person_name;?></td>
          <td><?=$Result->display_name;?></td>
          <td><?=$Result->area;?></td>
          <td><?=$Result->cost_per_sq_inch;?></td>
          <td><?=$Result->rate;?></td>
          <td><?=$Result->create_date;?></td>
          <td><a href="<?=base_url()?>index.php/backend/edit_framer/<?php echo  $Result->framer_code?>/<?php echo  $Result->framer_id?>">Edit Details</a></td>
          </tr>
         </tbody>
          <?php }?>
      </table>
...
var url="<?=base_url()?>index.php/backend/view_frames?vender_name="+vender_name;   
...
var url="<?=base_url()?>index.php/backend/view_frames?email_id="+email_id;   
window.location.href=url;
...
var url="<?=base_url()?>index.php/backend/view_frames?vender_code="+vender_code;   
window.location.href=url;
